...
namespace FishNet.Example
{
    ...
    public class NetworkHudCanvases : MonoBehaviour
    {
        ...
        #endregion
        [SerializeField] private Button buttonGhost;
        [SerializeField] private Button buttonHuman;
        public static int SelectedRole { get; private set; } = -1;
    ...
    private EventSystem _eventSystem;
        ...
        {
            if (_networkManager == null || SelectedRole == -1)
                return;
        ...
        {
            SelectedRole = role;
            Debug.Log($"✅ Rôle sélectionné : {(role == 0 ? "Fantôme" : "Humain")}");

            // Désactiver les boutons après sélection
            buttonGhost.gameObject.SetActive(false);
            buttonHuman.gameObject.SetActive(false);
        }
